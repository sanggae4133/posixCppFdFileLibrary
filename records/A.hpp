...
public:
    ...
    bool fromKv(const std::unordered_map<std::string, std::pair<bool, std::string>>& kv,
        ...
        auto n = kv.find("name");
        auto i = kv.find("id");
        if (n == kv.end() || i == kv.end()) { ec = std::make_error_code(std::errc::invalid_argument); return false; }
        if (!n->second.first || i->second.first) { ec = std::make_error_code(std::errc::invalid_argument); return false; }
